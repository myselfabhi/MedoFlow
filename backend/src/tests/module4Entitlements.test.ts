...
test('package remaining sessions are exposed in entitlement summary', async () => {
  ...
  try {
    ...
    const result = await membershipService.getPatientEntitlementSummary(
      'clinic-1',
      'patient-1'
    );
      ...
      callback({
        ...
        appointment: {
          ...
          create: async ({ data }: any) => ({
            ...
            provider: { id: 'provider-1', firstName: 'A', lastName: 'B', disciplines: [], user: { id: 'u1', name: 'Provider' } },
...
test('membership refund combinations fail safely', async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (err: any) =>
        err?.code === 'membership_refund_unsupported' && err?.statusCode === 400
    );
  } finally {
