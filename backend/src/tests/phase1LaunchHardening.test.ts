...
test('validateProductionEnv warns when STRIPE_WEBHOOK_SECRET is missing in production', () => {
  ...
  try {
    ...
    if (!process.env.STRIPE_WEBHOOK_SECRET) {
      console.warn('[WARN] STRIPE_WEBHOOK_SECRET is not set — Stripe webhook signature verification is DISABLED. ' +
...
test('handleAppointmentPaymentIntentSucceeded returns early if payment already PAID (no duplicate payment created)', async () => {
  ...
  try {
    ...
    assert.equal(paymentUpdateCalled, false, 'Should not update a payment record when already processed');
  } finally {
...
test('analytics resolveAnalyticsScope for PROVIDER ignores requestedProviderId and uses own provider ID', async () => {
  ...
  try {
    ...
    assert.equal(scope.providerId, 'provider-own', 'PROVIDER scope must be locked to own provider ID');
  } finally {
