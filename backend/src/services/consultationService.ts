...
export const createOrGetSession = async (
    ...
    if (!['CONFIRMED', 'COMPLETED'].includes(appointment.status)) {
        throwApi('Appointment must be confirmed or completed to start a consultation', 400, 'invalid_state');
    }
...
export const startRecording = async (
    ...
    if (recStatus !== 'NOT_STARTED' && recStatus !== 'FAILED') {
        throwApi(`Cannot start recording in recording state: ${session.recordingStatus}`, 400, 'invalid_state');
    }
...
export const startTranscription = async (
    ...
    const aiSession = await aiScribeService.createSession(
        visitRecordId,
        providerId,
        clinicId
    );
    ...
    await aiScribeService.uploadAudio(
        aiSession.id,
        providerId,
        clinicId,
        buffer,
