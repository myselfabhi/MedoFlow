...
export default function CheckoutPage() {
  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="container mx-auto py-24 text-center space-y-6">
        ...
        <Button onClick={() => router.push('/store')} className="rounded-full px-8">Go to Store</Button>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto py-12 max-w-4xl px-4 lg:px-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <h1 className="text-4xl font-black tracking-tight text-slate-900">Review Order</h1>
        {!isAuthenticated && (
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-100">
                {cart.items.map((item: any) => (
                  <div key={item.id} className="flex justify-between items-center p-8 hover:bg-slate-50/50 transition-colors">
                    <div className="flex gap-4">
                      <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center shrink-0">
                        {item.itemType === 'PRODUCT' ? <ShoppingBag className="h-8 w-8 text-slate-300" /> : <Package className="h-8 w-8 text-slate-300" />}
                      </div>
                      <div>
                        <div className="font-bold text-slate-900 text-lg">
                          {item.itemType === 'PRODUCT' ? item.product?.name : 
                           item.itemType === 'PACKAGE' ? item.package?.name : 
                           item.itemType === 'MEMBERSHIP' ? item.membership?.name : 
                           item.itemType === 'SERVICE' ? item.service?.name : item.name || 'Item'}
                        </div>
                        <div className="text-sm text-slate-500 font-medium">Quantity: {item.quantity} × ${Number(item.unitPrice).toFixed(2)}</div>
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-xl rounded-[2rem] overflow-hidden bg-primary text-white">
            <CardHeader className="p-8 border-none pb-0">
              <CardTitle className="text-white/60 text-sm font-black uppercase tracking-widest">Order Summary</CardTitle>
            </CardHeader>
              ...
              {!isAuthenticated ? (
                <div className="space-y-4 pt-4">
                  <p className="text-xs text-center text-white/60 leading-relaxed font-medium">
                    You are currently a guest. Please sign in to securely finalize your purchase and sync order to your account.
                  </p>
                  <Button 
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 leading-relaxed">Your transaction is encrypted and HIPAA compliant.</p>
            </div>
          </div>
        </div>
      </div>
