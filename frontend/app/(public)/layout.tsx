...
import { LayoutDashboard, ShoppingCart, CalendarPlus } from 'lucide-react';
...
function PublicShell({ children }: { children: React.ReactNode }) {
  ...
  const isStaff = isAuthenticated && user?.role !== 'PATIENT';

  return (
    <div className="min-h-screen bg-[#fafafa]">
      <header className="sticky top-0 z-40 border-b border-transparent bg-[#fafafa]/80 backdrop-blur-md">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            {!isLoading && isPatient && <PatientUserMenu />}

            {!isLoading && isPatient && (
              <button
                type="button"
                onClick={() => openPortal('booking')}
                className="group inline-flex items-center gap-2 rounded-full bg-gradient-to-br from-primary via-primary-700 to-accent-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:shadow-card hover:shadow-primary/30 active:scale-[0.98]"
              >
            ...
            {!isLoading && !isAuthenticated && (
              <Link href="/login" className="text-sm font-semibold text-slate-700 hover:text-primary transition-colors flex items-center gap-2">
                Login
              </Link>
            )}
