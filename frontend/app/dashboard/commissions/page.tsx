...
export default function CommissionsPage() {
  ...
  const saveRuleMutation = useMutation({
    mutationFn: async () => {
      ...
      if (ruleForm.itemType === 'SERVICE' && ruleForm.targetId) payload.serviceId = ruleForm.targetId;
      if (ruleForm.itemType === 'PRODUCT' && ruleForm.targetId) payload.productId = ruleForm.targetId;
      if (ruleForm.itemType === 'PACKAGE' && ruleForm.targetId) payload.packageId = ruleForm.targetId;
  ...
  const pendingRecordIds = records.filter((record) => record.status === 'PENDING').map((record) => record.id);
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'rules' && !isProvider && (
        <div className="grid gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
          <AppCard>
            <AppCardHeader>
              <AppCardTitle>{editingRuleId ? 'Edit Commission Rule' : 'Add Commission Rule'}</AppCardTitle>
            </AppCardHeader>
            <AppCardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setRuleForm((current) => ({ ...current, providerId: value }))}
                >
                  <SelectTrigger>
              ...
              {ruleForm.itemType !== 'ALL' && (
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setRuleForm((current) => ({ ...current, targetId: value }))}
                  >
                    <SelectTrigger>
              ...
              <div className="flex gap-2">
                ...
                >
                  {saveRuleMutation.isPending ? 'Saving...' : editingRuleId ? 'Update Rule' : 'Create Rule'}
                </AppButton>
                {editingRuleId && (
                  <AppButton
                    variant="outline"
          ...
          <AppCard>
            ...
            <AppCardContent>
              {rulesLoading ? (
                <div className="py-12 text-center text-sm text-slate-500">Loading commission rules…</div>
              ) : rules.length === 0 ? (
                <AppEmptyState
                      ...
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div className="space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <div className="font-medium text-slate-900">
                              {rule.provider ? `${rule.provider.firstName} ${rule.provider.lastName}` : 'All providers'}
                            </div>
                            <StatusBadge status={rule.isActive ? 'PAID' : 'DRAFT'} variant="invoice" />
                          </div>
                          <div className="text-sm text-slate-600">
                            {ITEM_TYPE_OPTIONS.find((option) => option.value === rule.itemType)?.label}
                            {' · '}
                            {ruleTargetLabel(rule)}
                          </div>
                          <div className="flex flex-wrap gap-2">
                            <AppBadge variant="secondary">{rule.commissionType === 'PERCENTAGE' ? 'Percentage' : 'Flat rate'}</AppBadge>
                        ...
                        <div className="flex gap-2">
                          ...
                          <AppButton
                            variant="ghost"
                            onClick={() =>
                              updateCommissionRule(rule.id, { isActive: !rule.isActive }).then(() => {
                                queryClient.invalidateQueries({ queryKey: ['commission-rules'] });
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-col gap-3 lg:flex-row">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setRecordFilters((current) => ({ ...current, status: value }))}
                  >
            ...
            <AppCardContent className="space-y-4">
              {!isProvider && pendingRecordIds.length > 0 && (
                <div className="flex justify-end">
                  ...
                  >
                    {markPaidMutation.isPending ? 'Updating...' : 'Mark Visible Pending Records as Paid'}
                  </AppButton>
                </div>
              )}

              <div className="text-sm text-slate-600">
                “Mark paid” only records local payout status. It does not trigger payroll or money movement.
              </div>

              {recordsLoading ? (
                <div className="py-12 text-center text-sm text-slate-500">Loading commission ledger…</div>
              ) : records.length === 0 ? (
                <AppEmptyState
                    ...
                    <TableBody>
                      {records.map((record) => (
                        <TableRow key={record.id}>
                          ...
                          <TableCell>{formatDate(record.earnedAt)}</TableCell>
                          <TableCell className="text-right">{formatMoney(record.basisAmount)}</TableCell>
                          <TableCell className="text-right font-semibold">{formatMoney(record.amount)}</TableCell>
                          <TableCell>
                            <StatusBadge status={record.status === 'PENDING' ? 'UNPAID' : record.status} variant="invoice" />
                          </TableCell>
                          <TableCell>{formatDate(record.paidOutAt)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
