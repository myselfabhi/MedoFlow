...
export default function PointOfSalePage() {
  ...
  const updateQuantity = (index: number, delta: number) => {
    setCart((current) => current.map((line, i) => {
      if (i === index) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (collectPaymentNow) {
        const amountToCollect = paymentAmount.trim().length > 0 ? Number(paymentAmount) : Number(finalizedInvoice.totalAmount);
      ...
      toast.success(collectPaymentNow ? 'Transaction completed and paid' : 'Invoice created and outstanding');
      setCart([]);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {services.filter(s => s.name.toLowerCase().includes(searchQuery.toLowerCase())).map(s => (
                ...
                >
                  <span className="font-bold text-slate-900 group-hover:text-primary-600 transition-colors truncate w-full">{s.name}</span>
                  <span className="text-xs text-slate-400 mt-1 uppercase font-bold tracking-tighter">Clinical Service</span>
                  <span className="mt-4 font-black text-slate-900">${Number(s.defaultPrice).toFixed(2)}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {products.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map(p => (
                ...
                >
                  <span className="font-bold text-slate-900 group-hover:text-primary-600 transition-colors truncate w-full">{p.name}</span>
                  <span className="text-xs text-slate-400 mt-1 uppercase font-bold tracking-tighter">Inventory: {p.inventoryItem?.quantityInStock ?? 0}</span>
                  <span className="mt-4 font-black text-slate-900">${Number(p.price).toFixed(2)}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {packages.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map(p => (
                <button 
                  key={p.id} 
                ...
                >
                  <span className="font-bold text-slate-900 group-hover:text-primary-600 transition-colors truncate w-full">{p.name}</span>
                  <span className="text-xs text-slate-400 mt-1 uppercase font-bold tracking-tighter">Pre-paid Sessions</span>
                  <span className="mt-4 font-black text-slate-900">${Number(p.price).toFixed(2)}</span>
                </button>
              ))}
            </div>
        ...
        <div className="w-[450px] bg-white border-l border-slate-200 flex flex-col shadow-2xl z-10">
          ...
          <div className="p-6 border-b bg-slate-50/50 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {selectedPatientId && (
                <button onClick={() => setSelectedPatientId('')} className="text-xs text-rose-600 font-bold hover:underline">Change</button>
              )}
            </div>
              ...
              <div className="p-4 bg-primary-50/50 rounded-2xl border border-primary-100 space-y-2 animate-in fade-in slide-in-from-top-2">
                ...
                {entitlements.activeMembershipBenefit && (
                  <p className="text-sm font-medium text-primary-900">
                    {entitlements.activeMembershipBenefit.membershipName} Member ({Number(entitlements.activeMembershipBenefit.serviceDiscountPercent)}% off)
                  </p>
                )}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              {cart.length > 0 && (
                <button onClick={() => setCart([])} className="text-xs text-slate-400 hover:text-rose-600 transition-colors font-bold uppercase tracking-tighter">Reset</button>
              )}
            </div>
            
            {cart.map((item, index) => (
              <div key={`${item.itemType}-${item.id}-${index}`} className="flex gap-4 p-4 bg-slate-50/50 rounded-2xl border border-slate-100 relative group transition-all hover:bg-white hover:shadow-sm">
                <button 
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateQuantity(index, -1)} className="w-7 h-7 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-50 shadow-sm"><Minus className="h-3 w-3" /></button>
                      <span className="text-xs font-black w-6 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(index, 1)} className="w-7 h-7 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-50 shadow-sm"><Plus className="h-3 w-3" /></button>
                    </div>
                    <p className="text-sm font-black text-slate-900">${(item.quantity * item.unitPrice).toFixed(2)}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="p-8 border-t bg-slate-50/30 space-y-6">
            <div className="space-y-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                />
                <label htmlFor="collectNow" className="text-sm font-bold text-slate-700 cursor-pointer">Record payment now</label>
              </div>
            ...
            <div className="pt-2 border-t border-slate-100">
              <div className="flex justify-between items-center mb-6">
                <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Est. Total</span>
